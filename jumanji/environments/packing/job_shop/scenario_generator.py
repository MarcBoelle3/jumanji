...
class FromDataSetScenarioGeneratorParallel(ScenarioGenerator):
    """Scenario generator that loads instances from a pre-existing dataset stored in a .npy file.
    It uses the key as an index to select which instance to serve from the dataset.
    """
    ...
    def __call__(self, key: chex.PRNGKey, num_jobs: int, num_machines: int) -> Scenario:
        """Returns a scenario from the loaded dataset based on the given key as an index.

        Args:
            key: A scalar array that is used as the index to select the instance.
            num_jobs: Unused number of jobs.
            num_machines: Unused number of machines.

        Returns:
            A `Scenario` object for the selected instance.
        """
        ...
        ops_machine_ids = self.dataset[instance_idx, 1, :, :].astype(jnp.int32) - 1
        
        # for Taillard instances, num_ops_per_job is always equal to num_machines
        num_ops_per_job = jnp.full(
            (self.max_num_jobs,), self.max_num_ops, dtype=jnp.int32
        )
