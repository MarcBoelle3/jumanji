...
def get_critical_operations(
    ...
    """Get operations that are on the critical path, based on the earliest and latest start time of each operation.
    Critical operations have the property that est[op] == lst[op], ie they cannot be moved in time
    without changing the makespan.


    Args:
        est: array of earliest start times, shape (max_num_jobs * max_num_ops + 2,) including source and target nodes
        lst: array of latest start times, shape (max_num_jobs * max_num_ops + 2,) including source and target nodes
        adj_mat_mc: adjacency matrix of the machine constraint graph, shape (max_num_jobs * max_num_ops + 2, max_num_jobs * max_num_ops + 2)
    ...
    # (is_on_critical_path, num_critical_block, is_left, is_right, left_neighbor, right_neighbor, left_end, right_end)
    ...
    def update_critical_block_with_pair(
        ...
        """Update the critical block information based on the critical block pair.
           During one update, if the pair (start_idx, end_idx) is valid, we update the critical block information
           as follows:
...
def get_action_mask_N5(critical_block_info: chex.Array, max_num_ops: int) -> chex.Array:
    ...
    # Mask operations that are both left and right ends (no possible action)
    ...
    right_valid = jnp.logical_and(is_left_end_only, job_idx != right_job_idx)
    left_valid = jnp.logical_and(is_right_end_only, job_idx != left_job_idx)

    # Set mask to True for right neighbor of left end and left neighbor of right end
    # Provided that the neighbor is not from the same job
    action_mask = jnp.zeros((num_ops_total, 2), dtype=jnp.bool_)
    action_mask = action_mask.at[:, 1].set(right_valid)
    action_mask = action_mask.at[:, 0].set(left_valid)

    return action_mask


def get_action_mask_N6(critical_block_info: chex.Array, max_num_ops: int) -> chex.Array:
    ...
    # Extract internal operations of critical blocks (not left or right end)
    is_critical = critical_block_info[:, 0]
    is_left_end = critical_block_info[:, 2]
    is_right_end = critical_block_info[:, 3]
    is_internal = is_critical & ~is_left_end & ~is_right_end  # shape (num_ops_total,)
    is_left_only = is_critical & is_left_end & ~is_right_end

    # Obtain jobs indices
    left_ends = critical_block_info[:, 6]
    right_ends = critical_block_info[:, 7]

    ops_idx = jnp.arange(num_ops_total)
    job_idx = ops_idx // max_num_ops
    right_job_idx = right_ends // max_num_ops
    left_job_idx = left_ends // max_num_ops

    # Get valid internal operations
    # To authorize swaps when CB has 2 operations, we need to add is_left_only to the mask
    # For now, there  is assymetry between left and right, otherwise there would be two actions resulting in the same schedule
    right_valid = jnp.logical_and(is_internal | is_left_only, job_idx != right_job_idx)
    left_valid = jnp.logical_and(is_internal, job_idx != left_job_idx)
    """jax.debug.print("is_internal for 0: {}", is_internal[0])
    jax.debug.print("right_valid for 0: {}", right_valid[0])
    jax.debug.print("left_valid for 0: {}", left_valid[0])
    jax.debug.print("any valid action in critical block: {}", jnp.any(right_valid | left_valid))"""
    # Set mask to True for right neighbor of internal operations provided that the neighbor is not from the same job
    action_mask = jnp.zeros((num_ops_total, 2), dtype=jnp.bool_)
    action_mask = action_mask.at[:, 1].set(right_valid)
    action_mask = action_mask.at[:, 0].set(left_valid)
...
def select_critical_swap_action(
    critical_block_info: chex.Array, chosen_op_idx: int, left_or_right: int, neighborhood: int
) -> chex.Array:
    """Select the action from the chosen operation index and the left or right neighbor.
        If the neighborhood is N5, the chosen operation is necessarily at the beginning/end of a critical block.
        The operation to swap it with is then the directly left/right operation in the critical block.
    action
        If the neighborhood is N6, the chosen operation is necessarily in the middle of a critical block.
        The operation to swap it with is then the left/right operation of the critical block.

        We assume that the left_or_right parameter is coherent with the critical_block_info array
        (i.e. if the chosen_op_idx is at the beginning of a critical block, left_or_right should be 1, indicating the right neighbor)

        Args:
            critical_block_info: array of critical block information, shape (num_ops_total, 6)
            chosen_op_idx: index of the chosen operation
            left_or_right: 0 for left, 1 for right neighbor
            neighborhood: 5 for N5, 6 for N6

        Returns: a pair of operations (i, j) that have to be switched.
        By convention, i has to start before j in the schedule.
    """
    ...
    # If right neighbor, return [chosen_op_idx, neighbor_idx] since chosen_op_idx is already before neighbor_idx
    """jax.debug.print("neighbor_idx: {}", neighbor_idx)
    jax.debug.print("chosen_op_idx: {}", chosen_op_idx)"""
...
def choose_random_action_N6(
    key: jax.random.PRNGKey, critical_block_info: chex.Array, max_num_ops: int
) -> chex.Array:
    """Choose an action based on uniform random scores and valid neighbor constraints.

    Returns: a pair of operation indices (i, j), with an edge from i to j
    (ie i is before j in the critical path) in the current solution.
    """
    num_ops_total = critical_block_info.shape[0]

    action_mask = get_action_mask_N6(critical_block_info, max_num_ops)

    # Generate random scores
    key, subkey = jax.random.split(key)
    scores = jax.random.uniform(subkey, shape=(num_ops_total, 2))

    # Mask out invalid actions
    masked_scores = scores * action_mask

    flat_idx = jnp.argmax(masked_scores)
    op_idx = jnp.unravel_index(flat_idx, masked_scores.shape)

    return select_critical_swap_action(critical_block_info, op_idx[0], op_idx[1], 6)


def choose_random_action_N5(
    key: jax.random.PRNGKey, critical_block_info: chex.Array, max_num_ops: int
) -> chex.Array:
    """Choose an action based on uniform random scores and valid neighbor constraints.
    ...
    action_mask = get_action_mask_N5(critical_block_info, max_num_ops)
    ...
    return select_critical_swap_action(critical_block_info, op_idx[0], op_idx[1], 5)
