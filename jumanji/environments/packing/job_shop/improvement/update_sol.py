...
def link_ops(
    ...
    """Link two operations in the adjacency matrix with a weight equal to the duration of the
    first op.
...
def get_predecessor(adj_matrix_mc: chex.Array, op: int) -> chex.Array:
    """Get the predecessor mask and weight indicating the predecessor of an operation in the
    adjacency matrix.
    ...
    Returns:
        - The predecessor mask of shape (max_num_jobs*max_num_ops+2,) indicating the
          predecessor of the operation.
...
def get_successor(adj_matrix_mc: chex.Array, op: int) -> chex.Array:
    """Get the successor mask and weight indicating the successor of an operation in the
    adjacency matrix.
    ...
    Returns:
        - The successor mask of shape (max_num_jobs*max_num_ops+2,) indicating the
