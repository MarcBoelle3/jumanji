...
def _find_earliest_permissible_insertion_point(
    ...
    machine_predecessor_end_times = jnp.concatenate([jnp.array([0.0]), machine_end_times[:-1]])
...
def schedule_op_in_earliest_slot(
    ...
    Adapted from "permissibleLeftShift" from
    https://github.com/zcaicaros/L2S/blob/main/env/permissible_LS.py .
    """
    ...
    machine_pred_end_time = jax.lax.cond(
        insertion_point == 0, lambda: 0.0, lambda: machine_end_times[insertion_point - 1]
    )
