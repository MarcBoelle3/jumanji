...
class RandomScheduleGenerator(ScheduleGenerator):
    ...
    def __call__(
        ...
        # Initialize machine constraints adjacency matrix using provided key
        # For method_id == 0, a random priority list will be generated inside.
        adj_mat_mc = self.init_adj_mat_mc(method_id, key, ops_machine_ids, ops_durations)
    ...
    def init_adj_mat_mc(
        ...
        def method_plist() -> chex.Array:
            return self.init_adj_mat_mc_with_plist(key, ops_machine_ids, ops_durations)
    ...
    def init_adj_mat_mc_with_plist(
        self, key: chex.PRNGKey, ops_machine_ids: chex.Array, ops_durations: chex.Array
    ) -> chex.Array:
        """
        ...
        Args:
            key: PRNGKey used to generate the job priority list.
        ...
        # Build priority list from key (permute only valid jobs)
        plist = jnp.arange(self.max_num_jobs)
        ...
        (_, _, final_adj_mat), _ = jax.lax.scan(
    ...
    def _run_fdd_mwr_simulation(
        ...
        def schedule_next_op(carry_state: tuple) -> tuple:
            ...
            updated_mch_sched, updated_starts, updated_ends, job_pred_ends = (
                schedule_op_in_earliest_slot(
                    op_id,
                    ops_durations,
                    mch_schedule[mch_id],
    ...
    def init_adj_mat_mc_with_fdd_mwr(
        ...
        final_schedule = self._run_fdd_mwr_simulation(ops_machine_ids, ops_durations, fdd)

        # 2. Build the adjacency matrix from the resulting schedule
        adj_matrix = self._build_adj_matrix_from_schedule(final_schedule, num_ops_total)
